...
//						 System.out.print(v.getIdx() +"\t");	// 고유번호 
				...
				case 3: // 회원가입 하기
					...
					while(true) {
						...
						System.out.println("동일한 닉네임이 존재합니다, 다른 닉네임을 입력해 주세요.");
					}
					vo.setName(name);
					System.out.print("회원 나이를 입력하세요 >> ");
					int age = scanner.nextInt();
					if(age < 20) {
						System.out.println(name+"님의 나이가 "+age+"이므로 가입할 수 없습니다.");
						System.out.println("미성년자는 가입 불가합니다.");
						break;
					}
					else vo.setAge(age);
				...
				case 4: // 수정하기
					...
					while(flag) {
						...
						if(vo.getName() == null) {
							...
							res = dao.setHoiwonUpdate(name,age,address,strGender);	// 회원 정보 수정 처리 메소드 호출
							if(res == 0) System.out.println(name + "님의 자료 수정이 실패되었습니다.");
							else System.out.println(name + "님의 자료가 수정되었습니다.");
						}
					}
					break;
				case 5: // 삭제처리하기
					...
					if(vo.getName() == null) {
						...
						dao.setDelete(name);	// 삭제처리 메소드 호츌
