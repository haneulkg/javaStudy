...
public class T01_FlowLayout extends JFrame {
	...
	public T01_FlowLayout() {
		...
		btnAnswer3.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// 옵션 'YES_NO_CANCEL_OPTION'인 경우는 '예/아니오/취소' >> '예:0' , '아니오:1', '취소:2' 값 반환
				int ans = JOptionPane.showConfirmDialog(null, "작업을 계속 하시겠습니까 ?", "선택창", JOptionPane.YES_NO_CANCEL_OPTION);
		...
		btnAnswer2.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// 옵션 'YES_NO_OPTION'인 경우는 '예/아니오' >> '예:0' , '아니오:1' 값 반환
				int ans = JOptionPane.showConfirmDialog(null, "작업을 계속 하시겠습니까 ?", "선택창", JOptionPane.YES_NO_OPTION);
				if(ans == 0) JOptionPane.showMessageDialog(null, "작업을 계속 진행합니다", "확인창", JOptionPane.INFORMATION_MESSAGE);
				else {
		...
		btnAnswer1.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// 옵션 'OK_CANCEL_OPTION'인 경우는 '확인/취소' >> '확인:0' , '취소:2' 값 반환
				int ans = JOptionPane.showConfirmDialog(null, "버튼을 선택하세요", "선택창", JOptionPane.OK_CANCEL_OPTION);
